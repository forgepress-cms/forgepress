...
export function useMedia(): MediaLibrary {
  ...
  async function refresh(): Promise<void> {
    assets.value = await attempt(pending, error, () => media.list(), assets.value)
    loaded = true
  }
  ...
  return {
    ...
    upload: async (files) => {
      const uploaded = await attempt(pending, error, async () => {
    ...
    remove: async (name) => {
      await attempt(pending, error, () => media.remove(name), undefined)
