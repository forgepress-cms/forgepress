...
import type { Forge, ForgeIdentity, OAuthTokens } from '../../src/forge/types'
import { computed, ref, shallowRef } from 'vue'
import { createForge } from '../../src/forge'
import { authorizeUrl, createChallenge, createState, createTokenGetter, createVerifier, exchange, expired, storedTokens } from '../../src/forge/oauth'
import { describe } from '../../src/forge/providers'
import { closePreview, openPreview } from '../../src/preview/state'
import { persist, repositoryCache, TOKEN_KEY } from '../../src/store'
...
async function current(): Promise<OAuthTokens> {
  if (tokens && expired(tokens)) {
    const stored = storedTokens(await store.read().catch(() => undefined))
...
async function remember(next: OAuthTokens): Promise<void> {
  tokens = next
...
async function adopt(next: OAuthTokens): Promise<boolean> {
  ...
  try {
    const candidate = createForge(config, createTokenGetter(config, current, remember))
