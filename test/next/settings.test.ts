...
describe('next editor settings', () => {
  it('reads the settings withForgePress defines at build time', async () => {
    ...
    const settings = { local: true, devServer: 'http://127.0.0.1:4321', provider, format: null, contentPath: '../.forgepress', media }

    vi.stubGlobal('__FORGEPRESS_SETTINGS__', JSON.stringify(settings))
    vi.resetModules()

    expect({ ...await import('../../src/next/settings') }).toEqual(settings)
  })
})
