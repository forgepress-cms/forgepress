...
function run(before: ElementType, after: ElementType, ...values: unknown[]) {
  return migrate(rows(...values), 'cover', before, after, locales)
}

const image = { type: 'image', optional: true } as ElementType
const images = { type: 'image', multiple: true, optional: true } as ElementType
...
describe('relations', () => {
  const one = { type: 'relation', component: 'author', optional: true } as ElementType
  const list = { type: 'relation', component: 'author', multiple: true, optional: true } as ElementType
  const other = { type: 'relation', component: 'tag', optional: true } as ElementType
  ...
  it('drops ids that point at another component', () => {
...
describe('types', () => {
  const text = { type: 'text', optional: true } as ElementType
  const richtext = { type: 'richtext', optional: true } as ElementType
  const number = { type: 'number', optional: true } as ElementType
  const video = { type: 'video', optional: true } as ElementType
...
describe('dynamic', () => {
  const wide = { type: 'dynamic', components: ['hero', 'textBlock'], optional: true } as ElementType
  const narrow = { type: 'dynamic', components: ['hero'], optional: true } as ElementType

  it('drops blocks whose component is no longer allowed', () => {
    const blocks = [{ type: 'hero', component: 'hero_1' }, { type: 'textBlock', component: 'textBlock_1' }]
...
describe('translation', () => {
  const plain = { type: 'text', optional: true } as ElementType
  const translated = { type: 'text', translate: true, optional: true } as ElementType
  ...
  it('migrates every locale in place', () => {
    const before = { type: 'text', translate: true, optional: true } as ElementType
    const after = { type: 'number', translate: true, optional: true } as ElementType
...
describe('required fields', () => {
  const optional = { type: 'text', optional: true } as ElementType
  const required = { type: 'text' } as ElementType
  const requiredEverywhere = { type: 'text', translate: true } as ElementType
